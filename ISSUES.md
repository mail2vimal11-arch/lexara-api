...
| QA-BUG-5 | P0 | `app/database/session.py` / prod DB | **`/v1/auth/register` 500s in production — root cause of the "Load failed" incident (CORS was only the messenger).** GH-runner diagnostic (run 33997282996) proved preflight + GET fine but POST register = HTTP 500; unhandled 500s bypass CORSMiddleware so browsers masked it as a CORS failure. Cause: prod `users` table predates newer model columns (plan_id etc.) — repo has NO alembic migrations and `create_all()` never alters existing tables. | RESOLVED | `reconcile_schema()` at startup: additive-only, idempotent — adds mapped-but-missing columns (nullable) and backfills scalar defaults. Plus temporary exception-class surfacing in auth 500s, and a pre-check for duplicate email (was an unhandled IntegrityError → 500). Tests reproduce the drift and prove the repair. |
| QA-BUG-4 | P1 | `app/config.py:42` | **CORS allow-list defaulted to the dead `lexrisk.com` domain — `lexara.tech` was never allowed.** Every browser call from the live site (register, login, analysis) failed preflight and surfaced as "Load failed". Invisible to curl/TestClient-based UAT, which bypasses CORS. Reported via user screenshot 2026-06-04. | RESOLVED | Default `allowed_origins` now includes `https://lexara.tech`, `https://www.lexara.tech`, and local dev origins; `.env.example` updated. Preflight verified approved for lexara.tech, rejected for unknown origins. **Prod note:** if the VPS `.env` sets `ALLOWED_ORIGINS`, it must be updated there too — env overrides the default. **Follow-up 2026-09-05:** the VPS `.env` did exactly that — registration stayed broken after deploy. `Settings.cors_origins()` now unions the site's own origins in unconditionally, so a stale env value can never lock the frontend out again. |
